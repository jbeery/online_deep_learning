...
def _get_kart_name(info: dict, instance_id: int) -> str:
    for key in ["kart_names", "karts", "players"]:
        values = info.get(key)
        if isinstance(values, list) and 0 <= instance_id < len(values):
            value = values[instance_id]
            if isinstance(value, dict):
                return str(value.get("name", value.get("kart", value.get("kart_name", instance_id))))
            return str(value)
        if isinstance(values, dict):
            value = values.get(str(instance_id), values.get(instance_id))
...
def _relative_position(kart: dict, ego: dict) -> tuple[str, str, str]:
...
def extract_kart_objects(
    ...
    for detection in info["detections"][view_index]:
        ...
        x1_clipped = max(0, min(img_width, x1_scaled))
        y1_clipped = max(0, min(img_height, y1_scaled))
        x2_clipped = max(0, min(img_width, x2_scaled))
        y2_clipped = max(0, min(img_height, y2_scaled))

        center = ((x1_clipped + x2_clipped) / 2, (y1_clipped + y2_clipped) / 2)
        objects.append(
            {
    ...
    if objects:
        ...
        center_idx = min(
            range(len(objects)),
            key=lambda i: (objects[i]["center"][0] - image_center[0]) ** 2
            + (objects[i]["center"][1] - image_center[1]) ** 2,
        )
        objects[center_idx]["is_center_kart"] = True
...
def extract_track_info(info_path: str) -> str:
    ...
    for key in ["track", "track_name", "map", "scene"]:
        value = info.get(key)
        if isinstance(value, dict):
            value = value.get("name", value.get("track"))
        if value is not None:
            return str(value)
...
def generate_dataset(data_dir: str = "data", split: str = "train", output_file: str | None = None):
    ...
    for info_file in sorted(split_path.glob("*_info.json")):
        ...
        for view_index in range(len(info["detections"])):
            ...
            for qa in generate_qa_pairs(str(info_file), view_index):
                qa_pairs.append(
                    {
    ...
    if output_file is None:
        output_file = split_path / "generated_qa_pairs.json"
    else:
