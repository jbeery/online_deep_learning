...
class CLIP(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        image_hidden = vision_output.last_hidden_state
        text_hidden = text_output.last_hidden_state

        image_feature = image_hidden.mean(dim=1)
        if attention_mask is None:
            text_feature = text_hidden[:, -1]
        else:
            mask = attention_mask.to(text_hidden.dtype).unsqueeze(-1)
            text_feature = (text_hidden * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1)

        image_feature = F.normalize(self.vision_projection(image_feature), dim=-1)
        text_feature = F.normalize(self.text_projection(text_feature), dim=-1)
...
def compute_clip_loss(
    ...
    """
    image_feature, text_feature, logit_scale = outputs
    logits_per_image = logit_scale * image_feature @ text_feature.T
    logits_per_text = logits_per_image.T
    target = torch.arange(image_feature.shape[0], device=image_feature.device)
