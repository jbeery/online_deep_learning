...
class TransformerPlanner(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        track = self.track_embed(track)

        side_ids = torch.cat(
            [
                torch.zeros(self.n_track, dtype=torch.long, device=track_left.device),
                torch.ones(self.n_track, dtype=torch.long, device=track_left.device),
            ],
            dim=0,
        )
        ...
        query = self.query_embed.weight[None, :, :]
