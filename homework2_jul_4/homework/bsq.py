...
class BSQ(torch.nn.Module):
    def __init__(self, codebook_bits: int, embedding_dim: int):
        ...
        self.down = torch.nn.Linear(embedding_dim, codebook_bits)
        self.up = torch.nn.Linear(codebook_bits, embedding_dim)
    ...
    def _index_to_code(self, x: torch.Tensor) -> torch.Tensor:
        bits = 2 ** torch.arange(self._codebook_bits, device=x.device)
        return 2 * (((x[..., None] // bits) % 2) > 0).float() - 1
...
class BSQPatchAutoEncoder(PatchAutoEncoder, Tokenizer):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        cnt = torch.bincount(
            self.bsq._code_to_index(z).flatten(),
